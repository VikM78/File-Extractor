# Universal File Extractor & Packer

[![Python 3.6+](https://img.shields.io/badge/python-3.6+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

Комплект инструментов для упаковки и распаковки файлов в текстовые шаблоны с поддержкой множества стилей комментариев.

## 📋 Описание

Проект состоит из трех основных компонентов:

- **`config.py`** - централизованная конфигурация стилей комментариев для 100+ типов файлов
- **`extract.py`** - распаковывает файлы из текстового шаблона с маркерами `start_my_file`/`end_my_file`
- **`pack.py`** - упаковывает файлы из папки в один текстовый шаблон

### 🔒 Безопасность по умолчанию

`pack.py` **автоматически уважает `.gitignore`** и не включает в шаблон:
- Файлы и папки из `.gitignore` (секреты, временные файлы, кэш)
- Системные файлы (`.DS_Store`, `Thumbs.db`)
- Временные файлы (`*.log`, `*.tmp`, `*.bak`)
- Кэш Python (`__pycache__/`, `*.pyc`)

## 🚀 Быстрый старт

```bash
# Скачать все файлы
wget https://raw.githubusercontent.com/VikM78/file-extractor/main/config.py
wget https://raw.githubusercontent.com/VikM78/file-extractor/main/extract.py
wget https://raw.githubusercontent.com/VikM78/file-extractor/main/pack.py

# Распаковать шаблон
python extract.py template.txt -o ./project

# Упаковать проект (безопасно - уважает .gitignore)
python pack.py ./project -o ./templates -n my_pack.txt
```

## 📖 Использование

### extract.py - Распаковка

```bash
python extract.py [SOURCE_FILE] [OPTIONS]
```

**Опции:**
- `SOURCE_FILE` - файл с шаблоном (по умолчанию: `template.txt`)
- `-o, --output PATH` - директория для извлечения (по умолчанию: `./extracted_files`)
- `-y, --yes` - автоматическое согласие
- `-v, --verbose` - подробный вывод
- `-h, --help` - справка

**Примеры:**
```bash
python extract.py                    # Интерактивный режим
python extract.py DATA1.txt          # Указать файл
python extract.py -o ./output        # Указать выходную папку
python extract.py -y                 # Автоматический режим
```

### pack.py - Упаковка с защитой секретов

```bash
python pack.py [SOURCE_DIR] [OPTIONS]
```

**Опции:**
| Опция | Описание |
|-------|----------|
| `SOURCE_DIR` | Папка с файлами (по умолчанию: `./ПАПКА`) |
| `-o, --output PATH` | Выходная папка (по умолчанию: `./output`) |
| `-n, --name NAME` | Имя выходного файла (по умолчанию: `DATA_compact.txt`) |
| `-e, --exclude PATTERN` | Исключить файлы по паттерну (можно несколько) |
| `-i, --include-ignored` | **Включить** файлы из `.gitignore` |
| `-a, --include-all` | Включить **ВСЕ** файлы (игнорировать .gitignore) |
| `-y, --yes` | Автоматическое согласие |
| `-v, --verbose` | Подробный вывод |
| `-h, --help` | Справка |

**Режимы работы:**

| Режим | Опция | Что упаковывается |
|-------|-------|-------------------|
| 🔒 **Безопасный** | (по умолчанию) | Все файлы, кроме указанных в `.gitignore` и стандартных исключений |
| ⚠️ **Расширенный** | `-i` | Все файлы, **включая** указанные в `.gitignore` |
| 🚨 **Полный** | `-a` | **Все** файлы (игнорирует `.gitignore` и стандартные исключения) |

**Примеры:**
```bash
# Безопасная упаковка (уважает .gitignore)
python pack.py ./my_project

# Упаковка с включением игнорируемых файлов
python pack.py ./my_project -i

# Полная упаковка (все файлы)
python pack.py ./my_project -a

# С пользовательскими исключениями
python pack.py ./my_project -e "*.pyc" -e "*.log"

# Включить игнорируемые, но исключить логи
python pack.py ./my_project -i -e "*.log"
```

## 📝 Формат шаблонов

**Простой формат:**
```
start_my_file file.txt
Content
end_my_file file.txt
```

**С комментариями (автоматически определяется):**

Python:
```python
# start_my_file script.py
print("Hello")
# end_my_file script.py
```

JavaScript:
```javascript
// start_my_file app.js
console.log("Hello");
// end_my_file app.js
```

HTML:
```html
<!-- start_my_file index.html -->
<html>...</html>
<!-- end_my_file index.html -->
```

CSS:
```css
/* start_my_file style.css */
body { color: red; }
/* end_my_file style.css */
```

SQL:
```sql
-- start_my_file query.sql
SELECT * FROM users;
-- end_my_file query.sql
```

INI:
```ini
; start_my_file config.ini
[Settings]
debug = true
; end_my_file config.ini
```

Lua:
```lua
--[[ start_my_file script.lua ]]
print("Hello")
--[[ end_my_file script.lua ]]
```

Haskell:
```haskell
{- start_my_file Main.hs -}
main = putStrLn "Hello"
{- end_my_file Main.hs -}
```

## 🔧 Поддерживаемые форматы

| Тип | Расширения | Стиль комментариев |
|-----|------------|-------------------|
| **Python, Shell, Ruby** | `.py`, `.sh`, `.rb` | `# ` |
| **JS, TS, Java, C/C++** | `.js`, `.ts`, `.java`, `.c` | `// ` |
| **HTML, XML, SVG** | `.html`, `.xml`, `.svg` | `<!-- -->` |
| **CSS, SCSS** | `.css`, `.scss` | `/* */` |
| **SQL** | `.sql`, `.psql`, `.mysql` | `-- ` |
| **INI, YAML, TOML** | `.ini`, `.yml`, `.toml` | `; ` или `# ` |
| **Lua** | `.lua` | `-- ` |
| **Haskell** | `.hs` | `{- -}` |
| **Pascal** | `.pas`, `.dpr` | `{ }` |
| **LaTeX** | `.tex` | `% ` |
| **PowerShell** | `.ps1` | `# ` |
| **Batch** | `.bat`, `.cmd` | `:: ` |
| **Dockerfile** | `Dockerfile` | `# ` |
| **Makefile** | `Makefile` | `# ` |

Полный список поддерживаемых форматов можно найти и расширить в файле `config.py`.

## 📁 Структура проекта

```
file-extractor/
├── config.py            # Конфигурация стилей комментариев
├── extract.py           # Распаковка файлов
├── pack.py              # Упаковка файлов (с защитой секретов)
├── README.md            # Документация
├── LICENSE              # Лицензия MIT
├── .gitignore           # Игнорируемые файлы
├── examples/            # Примеры использования
│   ├── sample_template.txt
│   └── sample_project/
│       ├── .gitignore
│       ├── .env.example
│       ├── hello.py
│       ├── config.ini
│       └── index.html
└── tests/               # Тесты
```

## 🔄 Рабочий процесс

### 1. Безопасная упаковка проекта

```bash
# Упаковать проект, исключая файлы из .gitignore
python pack.py ./my_project -o ./templates -n backup.txt

# Результат: .env, __pycache__/, *.log НЕ попадут в шаблон
```

### 2. Упаковка с секретами (для бэкапа)

```bash
# Включить файлы из .gitignore (например, для полного бэкапа)
python pack.py ./my_project -i -o ./backup -n full_backup.txt
```

### 3. Распаковка шаблона

```bash
# Извлечь все файлы из шаблона
python extract.py ./templates/backup.txt -o ./restored_project
```

### 4. Автоматизация

```bash
# Неинтерактивный режим с защитой секретов
python pack.py ./src -o ./backup -y
python extract.py ./backup/DATA_compact.txt -o ./restore -y
```

## 🛠 Расширение конфигурации

Все стили комментариев хранятся в файле `config.py`. Чтобы добавить новый тип файла:

1. Откройте `config.py`
2. Добавьте запись в словарь `COMMENT_STYLES`:

```python
COMMENT_STYLES = {
    # ... существующие стили ...
    '.new_ext': ('/* ', ' */'),  # Новый формат
}
```

3. Для специальных файлов без расширения добавьте в `SPECIAL_FILES`:

```python
SPECIAL_FILES = {
    # ... существующие файлы ...
    'newfile': ('# ', '# '),
}
```

4. Для добавления нового паттерна распознавания в `extract.py` добавьте в `EXTRACT_PATTERNS`:

```python
EXTRACT_PATTERNS = [
    # ... существующие паттерны ...
    (r'^\s*NEW\s*start_my_file\s+(.+)$', r'^\s*NEW\s*end_my_file\s+(.+)$', 'new_style'),
]
```

## 🔒 Безопасность и .gitignore

`pack.py` по умолчанию **уважает .gitignore** для защиты секретов:

### Что исключается по умолчанию

| Категория | Примеры |
|-----------|---------|
| **Секреты** | `.env`, `*.secret`, `credentials.json` |
| **Кэш Python** | `__pycache__/`, `*.pyc`, `*.pyo` |
| **Логи** | `*.log`, `*.tmp` |
| **Системные** | `.DS_Store`, `Thumbs.db` |
| **Резервные копии** | `*.bak`, `*.backup` |
| **Файлы IDE** | `.vscode/`, `.idea/` |
| **Git** | `.git/` |

### Контроль над исключениями

```bash
# Безопасный режим (по умолчанию)
python pack.py ./project

# Включить игнорируемые файлы
python pack.py ./project -i

# Полный бэкап (все файлы)
python pack.py ./project -a

# Свои исключения
python pack.py ./project -e "*.csv" -e "temp/*"
```

## 🐛 Решение проблем

| Проблема | Решение |
|----------|---------|
| **"No file blocks found!"** | Проверьте наличие маркеров `start_my_file`/`end_my_file` |
| **"File not found"** | Проверьте путь к файлу или папке |
| **"Permission denied"** | Проверьте права на запись в папку |
| **Секреты попали в шаблон** | Используйте `-e ".env"` или проверьте `.gitignore` |
| **.gitignore игнорируется** | Проверьте, что файл существует и правильно отформатирован |
| **Ошибка импорта config** | Убедитесь, что `config.py` находится в той же папке |

## 🧪 Тестирование

```bash
# Создать тестовый проект с секретами
mkdir test_project
echo "print('Hello')" > test_project/main.py
echo "SECRET_KEY=123" > test_project/.env
echo ".env" > test_project/.gitignore

# Упаковать (безопасно - .env будет исключен)
python pack.py test_project -o ./test_output -n test_pack.txt

# Проверить содержимое шаблона
grep ".env" ./test_output/test_pack.txt  # Должен быть пустой вывод

# Упаковать с включением игнорируемых
python pack.py test_project -i -o ./test_output -n test_with_secrets.txt

# Проверить содержимое
grep ".env" ./test_output/test_with_secrets.txt  # Должен найти .env
```

## 🤝 Вклад в проект

1. Форкните репозиторий
2. Создайте ветку (`git checkout -b feature/amazing`)
3. Закоммитьте изменения (`git commit -m 'Add feature'`)
4. Отправьте в форк (`git push origin feature/amazing`)
5. Откройте Pull Request

## 📄 Лицензия

MIT License

## 📞 Контакты

| | |
|---|---|
| **Автор** | Виктор Макаров |
| **Email** | vmakarov.kzn@gmail.com |
| **GitHub** | [@VikM78](https://github.com/VikM78) |

---

⭐ **Поставьте звезду, если проект полезен!**
